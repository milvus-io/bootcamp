...
def import_to_milvus(data,table_name):
    vectors = bc.encode(data)
    question_vectors = normaliz_vec(vectors.tolist())
    status, ids = milvus.add_vectors(table_name=table_name, records=question_vectors)
    print(status)
    time.sleep(60)
...
def create_milvus_table(table_name):
    param = {'table_name': table_name, 'dimension': 768, 'index_file_size':index_file_size, 'metric_type':metric_type}
    status = milvus.create_table(param)
    print(status)
    # index_param = {'index_type': IndexType.IVF_SQ8, 'nlist': nlist}
    # milvus.create_index(table_name,index_param)
...
def has_table(table_name):
    ...
    if not ok:
        # print("create table.")
        create_milvus_table(table_name)
...
def import_data(table_name, question_dir, answer_dir):
    question_data = read_data(question_dir)
    connect_milvus_server()
    has_table(table_name)
    ids = import_to_milvus(question_data,table_name)
    import_to_pg(table_name,ids,answer_dir)
        

def search_in_milvus(table_name, query_sentence):
    ...
    try:
        ...
        status,results = milvus.search_vectors(table_name=table_name, query_records=query_list, top_k=1, nprobe=32)
        if results[0][0].distance < 0.88:
    ...
    try:
        ...
        rows = pg_operating.search_in_pg(conn, cur, results[0][0].id, table_name)
        out_put = rows[0][1]
        return out_put
    except:
