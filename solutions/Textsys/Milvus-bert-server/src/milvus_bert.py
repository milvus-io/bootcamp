...
def import_to_pg(table_name,ids,title_file,version_file):  
    #建立数据库链接
    ...
    pg_operating.record_txt(ids,title_file, version_file)
...
def import_to_milvus(data,collection_name,milvus):
    ...
    status, ids = milvus.insert(collection_name=collection_name, records=title_vectors)
    print(status)
...
def import_data(collection_name, title_dir, version_dir):
    title_data = read_data(title_dir)
    milvus = Milvus(host=MILVUS_HOST, port=MILVUS_PORT)
    has_table(collection_name,milvus)
    ids = import_to_milvus(title_data,collection_name,milvus)
    import_to_pg(collection_name,ids,title_dir,version_dir)
