...
class MilvusHelper:
    ...
    # Create milvus collection if it does not exist
    ...
    def create_index(self, collection_name):
        try:
            if not self.client.has_collection(collection_name)[1]:
                raise Exception("Specified collection does not exist")
    ...
    def delete_collection(self, collection_name):
        try:
            ...
            if not status.code:
                LOGGER.debug("Successfully dropped collection: {}".format(collection_name))
                return status
            else:
                raise Exception(status.message)
        except Exception as e:
    ...
    # Get the size of a milvus collection
