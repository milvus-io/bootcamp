...
class MySQLHelper():
    ...
    def create_mysql_table(self, table_name):
        ...
        sql = "create table if not exists " + table_name + "(milvus_id TEXT, seq_class TEXT);"
        try:
    ...
    def load_data_to_mysql(self, table_name, data):
        self.test_connection()
        sql = "insert into " + table_name + " (milvus_id,seq_class) values (%s,%s);"
        try:
    ...
    def search_by_milvus_ids(self, ids, table_name):
        ...
        sql1 = "select * from " + table_name + " where milvus_id in (" + str_ids + ") order by field (milvus_id," + str_ids + ");"
        try:
