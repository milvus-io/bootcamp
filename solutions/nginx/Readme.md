# 使用Nginx实现对Milvus负载均衡

本示例主要展示如何利用Nginx实现对Milvus负载均衡，Nginx最常见的功能是服务器的负载均衡配置，通过使用Nginx分发请求，将不同请求分发给多个Milvus中，从而实现负载均衡。
...
3、编译前需要进行配置，将nginx安装到**/usr/local/ngnix**目录
...
stream {
    ...
    upstream milvus {
       server 192.168.1.85:19573; #为Milvus1的IP地址
       server 192.168.1.85:19538; #为Milvus2的Ip地址
    }
...
3、修改完Nginx配置文件之后，测试配置文件是否配置正确，先停止Nginx服务之后再重新启动
...
4、建立一个虚拟环境，安装Milvus对应版本的pymilvus ，运行**python3 test.py**脚本进行测试，然后使用如下命令查看nginx的日志文件
