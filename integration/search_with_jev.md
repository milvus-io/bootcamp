...
Markdown files are the editable source. Convert a changed tutorial with `uvx jupyter-switch tutorial.md` and keep the matching notebook in sync. Example outputs vary by model version. An HTTP or response-validation error stops execution; production applications should define an explicit fallback.
