<a href="https://colab.research.google.com/github/milvus-io/bootcamp/blob/master/bootcamp/RAG/search_with_jev/filter_search_context.ipynb"><img src="https://colab.research.google.com/assets/colab-badge.svg" alt="Open In Colab"/></a> · [View on GitHub](https://github.com/milvus-io/bootcamp/blob/master/bootcamp/RAG/search_with_jev/filter_search_context.ipynb)

# Filter retrieved context with Jev

Keep passages that provide evidence for the current question before building a generation prompt. No answer-generation call is needed to inspect the filtering step.
...
## Keep useful passages

Threshold the probabilities in Python; an empty result means more retrieval or abstention, not permission to invent evidence.

```python
query = "How do I install Atlas v2?"
candidates = retrieve(query, limit=5, filter_expr='version == "v2"')
answers = judge(
    {"query": query, "candidates": candidates},
    {
        f"keep_{i}": noul(
            f"Does `candidates[{i}].text` contain concrete evidence or an instruction useful for answering `query`, beyond merely mentioning the topic?"
        )
        for i in range(len(candidates))
    },
)
selected = [
    row for i, row in enumerate(candidates) if answers[f"keep_{i}"]["noul"] >= 0.7
]
context = "\n\n".join(row["text"] for row in selected)
print(context or "No sufficient context. Retrieve more or abstain.")
```
