<a href="https://colab.research.google.com/github/milvus-io/bootcamp/blob/master/bootcamp/RAG/search_with_jev/rerank_graph_relations.ipynb"><img src="https://colab.research.google.com/assets/colab-badge.svg" alt="Open In Colab"/></a> · [View on GitHub](https://github.com/milvus-io/bootcamp/blob/master/bootcamp/RAG/search_with_jev/rerank_graph_relations.ipynb)
...
## Score direct evidence and bridges

A useful path is Book A -> author B -> city D. Ask one independent question per relation; Python sorts the answers.

```python
query = "Where was the author of Book A born?"
candidates = retrieve("Book A author B born city publisher award", limit=4)
answers = judge(
    {"query": query, "relations": candidates},
    {
        f"relation_{i}": noul(
            f"Candidate relation: {candidates[i]['relation']}. Is this relation useful for answering `query`, either as direct birthplace evidence or as a bridge identifying the correct author? Example: Book -> author and author -> birthplace are useful; author -> award is not. Judge using the supplied relationship records only."
        )
...
ranked = sorted(
    ...
    key=lambda item: answers[f"relation_{item[0]}"]["noul"],
    reverse=True,
)
selected = [row for i, row in ranked if answers[f"relation_{i}"]["noul"] >= 0.5]
# Each toy relation has one source document; preserve ranked order when fetching it.
source_ids = list(dict.fromkeys(row["id"] for row in selected))
by_id = (
    {row["id"]: row for row in client.get(collection_name, source_ids)}
    if source_ids
    else {}
)
ordered_sources = [by_id[source_id] for source_id in source_ids]
print(
    "Ranked relations:",
    [(row["relation"], answers[f"relation_{i}"]["noul"]) for i, row in ranked],
)
print("Selected source text:", [row["text"] for row in ordered_sources])
```
